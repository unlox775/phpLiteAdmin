...
#				$cur_revision = 0.20;
				...
				///  UPGRADE from Version 0.1.0 to 0.1.1
				if ( $cur_revision == 0.10 ) {
					$this->__dbh->exec("ALTER TABLE file_tag ADD COLUMN
					                                mass_edit int NOT NULL DEFAULT 0");
				}
				///  Version 0.1.5
				if ( $INIT_DB_NOW || $cur_revision < 0.15 ) {
				...
				if ( $INIT_DB_NOW || $cur_revision < 0.21 ) {
					...
					$this->__dbh->exec("CREATE TABLE rlpt_file (
					...
					$this->__dbh->exec("CREATE INDEX file_idx ON rlpt_file(file)");
				...
				if ( $this_debug ) {
					///  Show DB Tables:
					$sth = $this->__dbh->query("SELECT * FROM sqlite_master WHERE type='table'");
					bug($sth->fetchAll(PDO::FETCH_ASSOC));

					$sth = $this->__dbh->query("SELECT * FROM rlpt_roll");
					bug($sth->fetchAll(PDO::FETCH_ASSOC));
				}
			}
		}
