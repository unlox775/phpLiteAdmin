...
*/
		...
		$proj_join = array();  foreach( $projects as $p ) $proj_join[ $p->project_name ] = $ctl->stage->dbh()->quote( $p->project_name );
		$roll_points = Ansible__RollPoint::get_where(array("(SELECT COUNT(*) FROM rlpt_project r WHERE roll_point.rlpt_id = r.rlpt_id AND r.project IN (". join(',',$proj_join) ." )) == ". count($proj_join),
														   "(SELECT COUNT(*) FROM rlpt_project r WHERE roll_point.rlpt_id = r.rlpt_id                                               ) == ". count($proj_join),
														   ),
													 false, 'creation_date DESC'
													 );
