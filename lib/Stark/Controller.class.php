...
class Stark__Controller {
	...
	public function __construct( $path, $path_config ) {
		...
		/// Detect url_prefix from controller_inc_base
		if ( isset( $path_config['controller_inc_base'] ) && ! isset( $path_config['url_prefix'] )
			 && is_dir($path_config['controller_inc_base']) && is_dir($_SERVER['DOCUMENT_ROOT'].dirname($path))
			) {
			$final = dirname($path);
			$current = realpath($_SERVER['DOCUMENT_ROOT'].$final);
			$goal = realpath($path_config['controller_inc_base']);
			$matched = false;
			...
			if ( $matched ) { $this->url_prefix = rtrim($final,'/').'/'; }
		}
	...
	public function load_controller_by_path($path) {
		...
		if ( $this->url_prefix != DIRECTORY_SEPARATOR && substr($path, 0, strlen($this->url_prefix)) == $this->url_prefix )
			$path = '/'.ltrim(substr($path, strlen($this->url_prefix)),DIRECTORY_SEPARATOR);
